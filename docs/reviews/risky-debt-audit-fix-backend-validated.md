...
Question: Audit the risky-debt lecture note and propose repairs with backend validation
...
Audited label list: `prop:risky-pricing`, `prop:interior-foc`

## Tool Uses

| Tool | Purpose | Status | Output contract | Arguments |
| --- | --- | --- | --- | --- |
| `audit_derivation_v2_label` | Generate local derivation audit evidence for `prop:risky-pricing`. | inconclusive | proof_audit_v2_result | `{'root': 'docs', 'label': 'prop:risky-pricing', 'paragraph_context': True, 'summary_only': True, 'backend': 'sympy'}` |
| `audit_derivation_v2_label` | Generate local derivation audit evidence for `prop:interior-foc`. | inconclusive | proof_audit_v2_result | `{'root': 'docs', 'label': 'prop:interior-foc', 'paragraph_context': True, 'summary_only': True, 'backend': 'sympy'}` |
| `propose_fix` | Translate audit evidence into conservative repair proposals. | diagnostic_only | high_level_workflow_result | `{'question': 'Audit the risky-debt lecture note and propose repairs with backend validation', 'evidence_count': 2, 'source': {'root': 'docs', 'labels': ['prop:risky-pricing', 'prop:interior-foc']}}` |
| `validate_proposed_fixes` | Attach deterministic backend-attempt accountability to concrete proposed fixes. | completed | proposal_fix_validation_summary | `{'policy': 'require_attempt_when_encodable', 'backend_order': ['lean', 'sage', 'sympy'], 'detail_count': 6}` |
...
Backend order: `['lean', 'sage', 'sympy']`
Validated details: 6
Status counts: attempted_not_certified=6
...
1. `split_derivation_step` for `obligation_3`
   ...
   Backend attempts: lean=not_encodable (diagnostic); sage=backend_unavailable (diagnostic); sympy=not_encodable (diagnostic)
   Evidence refs: proof_audit_v2:prop:risky-pricing:obligation_3
2. `split_derivation_step` for `obligation_2`
