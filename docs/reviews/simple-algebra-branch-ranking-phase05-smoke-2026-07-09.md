...
| `rank_repair_branches` | Rank assumption branches by recorded backend evidence, blocker specificity, source support, closure strength, and non-minimality. | `completed` | `repair_branch_ranking_result` | `{"ranked_branch_count": 1}` |
...
- Top branch: `branch_semantic_packet_eq_simple_0_typed_obligation_first`
- Rank `1`: `branch_semantic_packet_eq_simple_0_typed_obligation_first` outcome `scoped_proved`, score `119`
  - Components: `{'backend_certification': 100, 'closure_strength': 6, 'source_support': 15, 'blocker_specificity': 0, 'non_minimality_penalty': 2, 'score': 119, 'specific_blocker_count': 0, 'backend_attempt_count': 1, 'assumption_count': 3}`
  - Explanation: Ranked as scoped_proved: backend=100, closure=6, source=15, blocker_specificity=0, non_minimality_penalty=2.
...
- `branch_semantic_packet_eq_simple_0_typed_obligation_first` status `scoped_target_proved_not_document_proof`
  ...
  - Expansion records:
    ...
    - `formalization_route` status `not_selected_by_typed_route`: sage was not selected by the typed route hints for this branch.
    - `formalization_route` status `requires_formalization`: Lean requires explicit Lean source before direct checking can certify this branch.
    - `backend_attempt` status `proved`: sympy attempt with evidence certifying_backend and certification certified.
    - `blocker` status `blocking`: lean stub is not yet a certifying formalization.
