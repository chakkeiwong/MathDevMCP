...
"""Conservative reconciliation of explicit notation convention records."""
...
def reconcile_notation(
    ...
    *,
    ...
    for left_symbol, left in left_by_symbol.items():
        candidates = [
            (right_symbol, right)
            for right_symbol, right in right_by_symbol.items()
        ...
        if not candidates:
            ...
            continue
        right_symbol, right = candidates[0]
        matched_right.add(right_symbol)
    ...
    for right_symbol in sorted(set(right_by_symbol) - matched_right):
        unresolved.append(
            {
                "symbol": right_symbol,
                "side": right_context,
