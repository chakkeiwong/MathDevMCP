...
def test_security_scan_records_unavailable_tools_without_passing_them(tmp_path: Path) -> None:
    ...
    result = subprocess.run(
        ...
        capture_output=True,
        text=True,
        env={"PATH": "/usr/bin:/bin", "PYTHON_BIN": "/usr/bin/python3", "MATHDEVMCP_SECURITY_SCAN_MODE": "diagnostic", "MATHDEVMCP_SECURITY_ARTIFACT": str(output)},
    )
...
def test_security_scan_required_mode_blocks_unavailable_tools(tmp_path: Path) -> None:
    output = tmp_path / "security.json"
    result = subprocess.run(
        [str(ROOT / "scripts" / "security_scan.sh"), str(ROOT)],
        check=False,
    ...
    assert any(item["status"] == "not_available" for item in report["checks"].values())
